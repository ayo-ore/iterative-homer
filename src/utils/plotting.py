...
def plot_reweighting(
    ...
    lo, hi = np.quantile(np.hstack([sim, exp]), qlims)
    bins = (
    ...
    labels_rew = [f"{n}[{name_sim}]" for n in names_list]
    ...
    main_ax.legend(legend_objs, legend_labels, frameon=False, handlelength=1.4)
...
def plot_syst_calibration(
    ...
    bins_pull = np.linspace(-8, 8, num_bins_pull)
    xs = np.linspace(bins_pull[[0, 1]].mean(), bins_pull[[-1, -2]].mean(), 100)
